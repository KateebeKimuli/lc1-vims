...
const EMPTY_USER = {
  ...
  role: '', userStatus: 'active', phone: '', notes: ''
}
  ...
  async function saveLogo(dataUrl) {
    try {
      if (dataUrl) {
        ...
        try { await db.delete('settings', 'officialLogo') } catch {}
      }
      setLogoData(dataUrl)
      showToast(dataUrl ? 'Logo saved — appears on all documents' : 'Logo removed')
    } catch (err) {
        ...
        <div style={{ maxWidth: 700 }}>
          ...
          <div className="card" style={{ marginBottom: 20 }}>
            ...
            <p style={{ fontSize:13, color:'var(--c-text2)', marginBottom:14, lineHeight:1.6 }}>
              When a member forgets their password, generate a one-time token here.
              Give it to them verbally — they enter it on the login screen.
            </p>
            <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
              <div className="form-group" style={{ flex:1, minWidth:200 }}>
                <label className="form-label">Member's username</label>
                <input className="form-input" id="resetUsername" placeholder="Their username" />
              </div>
              <div style={{ alignSelf:'flex-end' }}>
                <button className="btn btn-gold" onClick={async () => {
                  const un = document.getElementById('resetUsername')?.value?.trim()
                  if (!un) { showToast('Enter the username first', 'error'); return }
                  if (!currentUser?.villageId) { showToast('No village context', 'error'); return }
                  try {
                    const token = await generateResetToken(currentUser.villageId, un)
                    showToast(`Token for ${un}: ${token}  (valid 24h)`, 'success', 8000)
